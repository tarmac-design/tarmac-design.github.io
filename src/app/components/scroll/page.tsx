  ...
  return (
    <>
      <p style={{ marginBottom: '1.5rem' }}>
        <a href="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-scroll--playground" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)', fontWeight: 500, fontSize: '14px' }}>Open in Storybook \u2192</a>
      </p>
      <h2>Description</h2>
      <p>The Scroll component is part of the TARMAC Design System. It provides a consistent, accessible, and reusable UI element for building interfaces across Delhivery products.</p>
      <h2>Availability</h2>
      <AvailabilityTable storybookUrl="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-scroll--playground" />
    </>
  );
}
  ...
  return (
    <>
      ...
      <table><thead><tr><th>Component</th><th>Description</th></tr></thead><tbody>
        <tr><td>Track</td><td>Background area where the scrollbar thumb moves</td></tr>
        <tr><td>Thumb</td><td>Draggable indicator showing current scroll position</td></tr>
        <tr><td>Arrow Buttons (optional)</td><td>Up/down or left/right arrows for incremental scrolling</td></tr>
      </tbody></table>
      <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-outline)', marginTop: '1.5rem', marginBottom: '0.5rem', background: '#fff' }}>
        <iframe src="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/iframe.html?id=tarmac-tds-tdsscrollbar--playground&viewMode=story&shortcuts=false" style={{ width: '100%', height: '300px', border: 'none', display: 'block' }} title="scroll example" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-forms" />
      </div>

      <h2>Sizes</h2>
      <table><thead><tr><th>Size</th><th>Description</th></tr></thead><tbody>
        <tr><td>Default</td><td>6px width for standard scrollbar appearance</td></tr>
        <tr><td>Thin</td><td>4px width for minimal visual footprint</td></tr>
      </tbody></table>

      <h2>States</h2>
      <table><thead><tr><th>State</th><th>Description</th></tr></thead><tbody>
        <tr><td>Idle (hidden)</td><td>Scrollbar is not visible when not scrolling</td></tr>
        <tr><td>Hover (visible)</td><td>Scrollbar appears when cursor enters the scroll area</td></tr>
        <tr><td>Active (dragging)</td><td>Thumb is being dragged by the user</td></tr>
      </tbody></table>
...
function GuidelinesTab() {
  return (
    <RoleToggle>
      ...
      <GuidelineImage title="When Not to Use \u2014 Scroll" slug="scroll" section="when-not-to-use" />
      <ul>
      ...
      <DoDont slug="scroll" doItems={['Follow the design system guidelines', 'Use consistent sizing and spacing', 'Ensure proper accessibility attributes', 'Test across different viewports']} dontItems={["Don't modify the component outside its API", "Don't use inconsistent styling", "Don't ignore accessibility requirements", "Don't override design tokens without approval"]} />
    </>
      ) : (
        <>
          <h2>Installation</h2>
          ...
          <h2>Import</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`import { TdsScrollbar } from '@tarmac/design-system';`}</code></pre>
          ...
          <h2>Basic Usage</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`<TdsScrollbar />`}</code></pre>
        </>
      )}
    </RoleToggle>
  );
...
export default function ScrollPage() {
  ...
  return (
    <PageShell title="Scroll" description="Scroll is a reusable component in the TARMAC Design System." tabs={tabs}>
      <OverviewTab />
    </PageShell>
  );
}
