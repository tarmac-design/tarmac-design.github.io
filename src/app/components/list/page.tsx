  ...
  return (
    <>
      <p style={{ marginBottom: '1.5rem' }}>
        <a href="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-list--playground" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)', fontWeight: 500, fontSize: '14px' }}>Open in Storybook \u2192</a>
      </p>
      <h2>Description</h2>
      ...
      <h2>Availability</h2>
      <AvailabilityTable storybookUrl="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-list--playground" />
    </>
  );
}
  ...
  return (
    <>
      ...
      <table><thead><tr><th>Component</th><th>Description</th></tr></thead><tbody>
        <tr><td>List Container</td><td>Wrapping element that holds all list items</td></tr>
        <tr><td>List Item</td><td>Individual row representing a single entry</td></tr>
        <tr><td>Leading Content</td><td>Optional icon, avatar, or checkbox before the label</td></tr>
        <tr><td>Trailing Content</td><td>Optional metadata, action icon, or control at the end</td></tr>
        <tr><td>Divider</td><td>Separator line between list items</td></tr>
      </tbody></table>
      <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-outline)', marginTop: '1.5rem', marginBottom: '0.5rem', background: '#fff' }}>
        <iframe src="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/iframe.html?id=tarmac-tds-listset--playground&viewMode=story&shortcuts=false" style={{ width: '100%', height: '300px', border: 'none', display: 'block' }} title="list example" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-forms" />
      </div>

      <h2>Sizes</h2>
      <table><thead><tr><th>Size</th><th>Description</th></tr></thead><tbody>
        <tr><td>Large</td><td>56px row height for touch-friendly interfaces</td></tr>
        <tr><td>Medium</td><td>48px row height for standard density layouts</td></tr>
        <tr><td>Small</td><td>40px row height for compact or data-dense views</td></tr>
      </tbody></table>

      <h2>States</h2>
      <table><thead><tr><th>State</th><th>Description</th></tr></thead><tbody>
        <tr><td>Default</td><td>Resting state with no interaction</td></tr>
        <tr><td>Selected</td><td>Item is currently selected with visual highlight</td></tr>
        <tr><td>Hover</td><td>Cursor hovering over a list item</td></tr>
    ...
    <RoleToggle>
      ...
      <DoDont slug="list" doItems={['Follow the design system guidelines', 'Use consistent sizing and spacing', 'Ensure proper accessibility attributes', 'Test across different viewports']} dontItems={["Don't modify the component outside its API", "Don't use inconsistent styling", "Don't ignore accessibility requirements", "Don't override design tokens without approval"]} />
        ...
        <>
          ...
          <h2>Import</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`import { ListSet } from '@tarmac/design-system';`}</code></pre>
          ...
          <h2>Basic Usage</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`<ListSet />`}</code></pre>
        </>
      )}
    </RoleToggle>
  );
