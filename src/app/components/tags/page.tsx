  ...
  return (
    <>
      <p style={{ marginBottom: '1.5rem' }}>
        <a href="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-tags--playground" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)', fontWeight: 500, fontSize: '14px' }}>Open in Storybook \u2192</a>
      </p>
      <h2>Description</h2>
      ...
      <h2>Availability</h2>
      <AvailabilityTable storybookUrl="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-tags--playground" />
    </>
  );
}
  ...
  return (
    <>
      ...
      <table><thead><tr><th>Component</th><th>Description</th></tr></thead><tbody>
        <tr><td>Container</td><td>Wrapping element with rounded styling for the tag</td></tr>
        <tr><td>Label</td><td>Text content describing the tag value</td></tr>
        <tr><td>Leading Icon</td><td>Optional icon before the label for visual context</td></tr>
        <tr><td>Remove Button</td><td>Optional close icon to remove the tag</td></tr>
      </tbody></table>
      <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-outline)', marginTop: '1.5rem', marginBottom: '0.5rem', background: '#fff' }}>
        <iframe src="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/iframe.html?id=tarmac-tds-chip--playground&viewMode=story&shortcuts=false" style={{ width: '100%', height: '300px', border: 'none', display: 'block' }} title="tags example" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-forms" />
      </div>

      <h2>Sizes</h2>
      <table><thead><tr><th>Size</th><th>Description</th></tr></thead><tbody>
        <tr><td>Large</td><td>32px height for prominent tag placements</td></tr>
      ...
      <table><thead><tr><th>State</th><th>Description</th></tr></thead><tbody>
        ...
        <tr><td>Selected</td><td>Tag is actively selected with visual highlight</td></tr>
        <tr><td>Hover</td><td>Cursor hovering over the tag</td></tr>
  ...
  return (
    <RoleToggle>
      {(role) => role === 'designer' ? (
        <>
      <h2>When to Use</h2>
          <GuidelineImage title="Tags usage overview" slug="tags" section="usage" />
      ...
      <DoDont slug="tags" doItems={['Follow the design system guidelines', 'Use consistent sizing and spacing', 'Ensure proper accessibility attributes', 'Test across different viewports']} dontItems={["Don't modify the component outside its API", "Don't use inconsistent styling", "Don't ignore accessibility requirements", "Don't override design tokens without approval"]} />
        ...
        <>
          ...
          <h2>Import</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`import { Tags } from '@tarmac/design-system';`}</code></pre>
          ...
          <h2>Basic Usage</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`<Tags />`}</code></pre>
        </>
      )}
    </RoleToggle>
  );
