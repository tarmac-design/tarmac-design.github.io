  ...
  return (
    <>
      <p style={{ marginBottom: '1.5rem' }}>
        <a href="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-slider--playground" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)', fontWeight: 500, fontSize: '14px' }}>Open in Storybook \u2192</a>
      </p>
      <h2>Description</h2>
      ...
      <h2>Availability</h2>
      <AvailabilityTable storybookUrl="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-slider--playground" />
    </>
  );
}
  ...
  return (
    <>
      ...
      <table><thead><tr><th>Component</th><th>Description</th></tr></thead><tbody>
        <tr><td>Track</td><td>Horizontal bar representing the full value range</td></tr>
        <tr><td>Fill</td><td>Colored portion from minimum to the current value</td></tr>
        <tr><td>Thumb</td><td>Draggable circular handle for setting the value</td></tr>
        <tr><td>Value Label</td><td>Tooltip or label showing the current numeric value</td></tr>
        <tr><td>Min/Max Labels</td><td>Text indicating the range boundaries</td></tr>
      </tbody></table>
      <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-outline)', marginTop: '1.5rem', marginBottom: '0.5rem', background: '#fff' }}>
        <iframe src="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/iframe.html?id=tarmac-tds-slider--playground&viewMode=story&shortcuts=false" style={{ width: '100%', height: '300px', border: 'none', display: 'block' }} title="slider example" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-forms" />
      </div>

      <h2>Sizes</h2>
      <table><thead><tr><th>Size</th><th>Description</th></tr></thead><tbody>
        <tr><td>Default</td><td>Standard slider size that spans its container</td></tr>
      </tbody></table>

      <h2>States</h2>
      <table><thead><tr><th>State</th><th>Description</th></tr></thead><tbody>
        <tr><td>Default</td><td>Resting state with thumb at current position</td></tr>
        <tr><td>Hover</td><td>Thumb enlarges or highlights on cursor hover</td></tr>
        <tr><td>Active (dragging)</td><td>User is actively dragging the thumb</td></tr>
        <tr><td>Disabled</td><td>Non-interactive, visually muted</td></tr>
        <tr><td>Range (two thumbs)</td><td>Two thumbs defining a value range</td></tr>
      </tbody></table>
    ...
    <RoleToggle>
      ...
      <DoDont slug="slider" doItems={['Follow the design system guidelines', 'Use consistent sizing and spacing', 'Ensure proper accessibility attributes', 'Test across different viewports']} dontItems={["Don't modify the component outside its API", "Don't use inconsistent styling", "Don't ignore accessibility requirements", "Don't override design tokens without approval"]} />
        ...
        <>
          ...
          <h2>Import</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`import { Slider } from '@tarmac/design-system';`}</code></pre>
          ...
          <h2>Basic Usage</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`<Slider />`}</code></pre>
        </>
      )}
    </RoleToggle>
  );
