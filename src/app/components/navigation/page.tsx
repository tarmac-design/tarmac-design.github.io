  ...
  return (
    <>
      <p style={{ marginBottom: '1.5rem' }}>
        <a href="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-navigation--playground" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)', fontWeight: 500, fontSize: '14px' }}>Open in Storybook \u2192</a>
      </p>
      <h2>Description</h2>
      ...
      <h2>Availability</h2>
      <AvailabilityTable storybookUrl="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/index.html?path=/story/tarmac-tds-navigation--playground" />
    </>
  );
}
  ...
  return (
    <>
      ...
      <table><thead><tr><th>Component</th><th>Description</th></tr></thead><tbody>
        <tr><td>Nav Container</td><td>Wrapping element that holds all navigation items</td></tr>
        <tr><td>Nav Item</td><td>Individual clickable navigation entry</td></tr>
        <tr><td>Icon</td><td>Visual icon representing the navigation destination</td></tr>
        <tr><td>Label</td><td>Text label describing the navigation item</td></tr>
        <tr><td>Active Indicator</td><td>Visual highlight showing the currently active route</td></tr>
      </tbody></table>
      <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--color-outline)', marginTop: '1.5rem', marginBottom: '0.5rem', background: '#fff' }}>
        <iframe src="https://tarmac-storybook-dev.pntrzz.com/storybook/sb/iframe.html?id=tarmac-tds-sidenavigation--playground&viewMode=story&shortcuts=false" style={{ width: '100%', height: '300px', border: 'none', display: 'block' }} title="navigation example" loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups allow-forms" />
      </div>

      <h2>Sizes</h2>
      <table><thead><tr><th>Size</th><th>Description</th></tr></thead><tbody>
        <tr><td>Expanded</td><td>240px width with icon and label visible</td></tr>
        <tr><td>Collapsed</td><td>60px icon-only mode for compact layouts</td></tr>
      </tbody></table>

      <h2>States</h2>
      <table><thead><tr><th>State</th><th>Description</th></tr></thead><tbody>
        <tr><td>Default</td><td>Resting state with no interaction</td></tr>
        <tr><td>Active</td><td>Currently selected navigation item with indicator</td></tr>
        <tr><td>Hover</td><td>Cursor hovering over a navigation item</td></tr>
        <tr><td>Collapsed</td><td>Navigation is in icon-only compact mode</td></tr>
      </tbody></table>
    ...
    <RoleToggle>
      ...
      <DoDont slug="navigation" doItems={['Follow the design system guidelines', 'Use consistent sizing and spacing', 'Ensure proper accessibility attributes', 'Test across different viewports']} dontItems={["Don't modify the component outside its API", "Don't use inconsistent styling", "Don't ignore accessibility requirements", "Don't override design tokens without approval"]} />
        ...
        <>
          ...
          <h2>Import</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`import { Navigation } from '@tarmac/design-system';`}</code></pre>
          ...
          <h2>Basic Usage</h2>
          <pre style={{ background: 'var(--color-surface-dim)', padding: '16px', borderRadius: '8px', fontSize: '13px', overflow: 'auto' }}><code>{`<Navigation />`}</code></pre>
        </>
      )}
    </RoleToggle>
  );
