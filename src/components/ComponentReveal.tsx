...
export default function ComponentReveal() {
  ...
  const frameY = progress <= 0.08
    ? 120 - clamp(progress / 0.08) * 120
    : progress >= 0.92
      ? -clamp((progress - 0.92) / 0.08) * 120
      : 0;
  ...
  return (
    <div ref={spacerRef} style={{
      ...
      {/* Text revealed BEHIND the frame as it scrolls up */}
      <div style={{
        ...
        textAlign: 'center', zIndex: 3, padding: '0 24px',
        paddingTop: '28vh',
      }}>
        <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 800, color: isDark ? '#fff' : '#101012', margin: '0 0 16px', letterSpacing: '-0.02em' }}>
          Production-ready components.
        </h2>
        <p style={{ fontSize: 15, color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(16,16,18,0.55)', maxWidth: 480, margin: '0 0 32px', lineHeight: 1.6 }}>
          Every component ships with Storybook demos, accessibility built in, design tokens, and full documentation.
        </p>
        <a href="/components/accordion" style={{
          background: '#ED1B36', color: '#fff', fontWeight: 700,
          fontSize: 14, padding: '12px 24px', borderRadius: 8,
          textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8,
        }}>
          View all components <span>→</span>
        </a>
      </div>

      {/* Browser frame on top — covers the text until it scrolls away */}
      <div style={{
        position: isPinned ? 'fixed' : 'absolute',
        top: isPinned ? 0 : (isBelow ? 0 : undefined),
        bottom: (!isPinned && !isBelow) ? 0 : undefined,
